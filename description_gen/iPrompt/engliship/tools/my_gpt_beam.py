...
def get_persona():
    ...
    res_l = res.split('\n')
    res_l_f = [sent[2:] for sent in res_l]
...
def extract_person(persona_l):
    start_index = 0
    persona_l_e = list()
    for string in persona_l[:persona_size]:
...
def extract_ent(persona_l):
    ...
    for string in persona_l[:persona_size]:
        ...
        end_index = string.find(")")
        if end_index != -1:  # 如果找到了 "("
            result = string[start_index+1:end_index]  # 提取字段
        else:
            result = string[start_index+1:]  # 如果没有找到 "("，则提取到字符串末尾
        cut_index = result.find(":")
        persona_l_e.append(result[:cut_index].strip())
        persona_l_v.append(result[cut_index+1:].strip())
    return persona_l_e,persona_l_v


def regex_based_filtering(persona_des_tot):
    result = []

    for i in range(persona_size):
        sentence = persona_des_tot[i]

        pattern = '(?P<utter>.*) [\(|\[](?P<attr>.*): (?P<value>.*)[\)|\]]'  # [] case should be possible
        compiled_regex = re.compile(pattern)

        matched = compiled_regex.match(sentence)
...
def explicit_filtering(sentences):
    results = []
...
def persona_category_filtering(sentences):
    # 加载 zero-shot 分类管道
    ...
    for sentence in sentences:
        ...
        output = classifier(utter, attr)
        if output['scores'][0] < th:
            continue
...
def duplication_filtering( sentences):
    results = []
    dup_results = {}
    for sentence in sentences:
        ...
        if utter in dup_results.keys():
            if dup_results[utter][1] != value:
                print(
                    f'Warning: entity value mismatch occurs! Previous value = {dup_results[utter][1]}, Current value = {value} of utterance = {utter}.')
                continue
...
def do_filtering(persona_des_tot):
    regex_filter_results = regex_based_filtering(persona_des_tot)

    explicit_filter_results = explicit_filtering(regex_filter_results)

    persona_category_filter_results = persona_category_filtering(explicit_filter_results)

    duplication_filter_results = duplication_filtering(persona_category_filter_results)
...
def calculate_perplexity(model, tokenizer, input, output):
    ...
    while (counter < output_length and (done == False)):
        ...
        # print(output_tokens_tensor[counter])
        context_tokens_tensor = torch.tensor(tokenizer.convert_tokens_to_ids(context_tokens))
...
def main():
    ...
    # 生成多组的personas进行筛选
    persona_des_beam = []
    for i in range(beam_persona):
        persona_des_tot = get_persona()
        # persona_des_pure = extract_person(persona_des_tot)
        # persona_des_ent,persona_des_val  = extract_ent(persona_des_tot)


        # 做filtering
        persona_fil = do_filtering(persona_des_tot)
        persona_fil_utter = [persona['utter'] for persona in persona_fil]
        persona_des_beam.append(persona_fil_utter)
        print(persona_fil_utter)


    persona_score = list()
    # 构造问题
    c= 0
    for persona_des in persona_des_beam:
        persona = ''
        for persona_d in persona_des:
            persona += persona_d + ";"
        persona = persona[:-1]
        print(persona)
        print("set:")
    ...
    print("max_index: %s, max_value: %s" % (max_index, persona_des_beam[max_index]))
