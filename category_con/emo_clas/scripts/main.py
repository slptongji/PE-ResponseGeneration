...
class BatchPreprocessor(object):
    ...
    def llm_context_vector_preprocess(self):
        ...
        for conv_id, conv in all_convs.items():
            ...
            if self.model_configs.llm_aggregate_method == 'accwr_selfattn':
                ...
                padding_word_masked = torch.BoolTensor(
                    [[False] * e.shape[0] + [True] * (max_sent_len - e.shape[0]) for e in llm_context_vectors])
                new_vect_dict[conv_id + "_padding_word_masked"] = padding_word_masked

                llm_context_vectors = torch.stack(
                    [F.pad(e, (0, 0, 0, max_sent_len - e.shape[0]), "constant", 0) for e in llm_context_vectors], dim=0)
    ...
    def __call__(self, batch):
        ...
        if self.model_configs.speaker_description:
            ...
            sp_characteristic_word_ids = {'len_sp_desc': len(all_speaker_descriptions),
                                          'all_speaker_descriptions': all_speaker_descriptions_idx}
        # ========

        # utterance vectorizer
        # v_single_sentences = self._encoding(sample['sentences'])
        contextual_sentences_ids = self.tokenizer(raw_sentences_flatten_spdesc, padding='longest', max_length=512,
                                                  truncation=True, return_tensors='pt')
        sent_indices, word_indices = torch.where(
            contextual_sentences_ids['input_ids'][n_spdesc:] == self.separate_token_id)
        ...
        cur_sentence_indexes_masked = torch.BoolTensor(contextual_sentences_ids['input_ids'][n_spdesc:].shape).fill_(
            False)
        ...
        return (contextual_sentences_ids, torch.LongTensor(labels), padding_utterance_masked, intra_speaker_masekd_all,
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, q_input, k_input, v_input, attn_mask=None, key_padding_mask=None):
        ...
        attention_scores = attention_scores + attention_mask_mt.unsqueeze(0).reshape(
            attention_scores.shape) if attention_mask_mt is not None else attention_scores
...
class EmotionClassifier(pl.LightningModule):
    def __init__(
        ...
        self.model = AutoModel.from_pretrained("../finetuned_llm/roberta")  # a pretraied Roberta model
        ...
        nhead = 8
        num_layer = 1
        self.context_modeling = nn.TransformerEncoder(
            encoder_layer=nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, batch_first=True),
            num_layers=num_layer)  # using transformer model in here
        self.pos_encoding = PositionalEncoding(
            d_model)  # position encoding => for utterance positions (e.g., u_1, u_2, ...) in conversation.
        ...
        if model_configs.intra_speaker_context:
            ...
            self.intra_speaker_modeling = BertSelfAttention(int(d_model / 2), num_heads=nhead,
                                                            dropout=model_configs.dropout, batch_first=True)
            self.iq = nn.Linear(d_model, int(d_model / 2))
            self.output_layer_intra = nn.Linear(int(d_model / 2), model_configs.num_labels)

            self.inter_speaker_modeling = BertSelfAttention(int(d_model / 2), num_heads=nhead,
                                                            dropout=model_configs.dropout, batch_first=True)
        ...
        if model_configs.llm_context:
            ...
            if model_configs.llm_aggregate_method == 'accwr_selfattn':
                self.llm_attention_modeling = nn.MultiheadAttention(llm_model, num_heads=8, dropout=0.2,
                                                                    batch_first=True)
        ...
        if model_configs.speaker_description:
            if model_configs.spdesc_aggregate_method == 'attn':
                self.speaker_character_attention_modeling = nn.MultiheadAttention(d_model, num_heads=8, dropout=0.2,
                                                                                  batch_first=True)
        ...
        self.loss_computation = torch.nn.CrossEntropyLoss(ignore_index=-1, label_smoothing=0.1, weight=torch.Tensor(
            self.model_configs.class_weights))  # label_smoothing
    ...
    def aggregate_sentence_h_state(self, cls_hidden_state, h_words_hidden_states, cur_sentence_indexes_masked):
        if self.model_configs.sent_aggregate_method == 'average':
            ...
            sentence_vectors = torch.sum(h_words_hidden_states * cur_sentence_indexes_masked.unsqueeze(-1), dim=1) * (
                        1 / torch.sum(cur_sentence_indexes_masked, dim=1)).unsqueeze(-1)
            ...
            sentence_vectors_avg = torch.sum(h_words_hidden_states * cur_sentence_indexes_masked.unsqueeze(-1),
                                             dim=1) * (1 / torch.sum(cur_sentence_indexes_masked, dim=1)).unsqueeze(-1)
            ...
            sentence_vectors = torch.sum(h_words * cur_sentence_indexes_masked.unsqueeze(-1), dim=1) * (
                        1 / torch.sum(cur_sentence_indexes_masked, dim=1)).unsqueeze(-1)
    ...
    def training_step(self, batch, batch_idx, return_y_hat=False):
        ...
        sentence_vectors = self.aggregate_sentence_h_state(bert_out.pooler_output[n_speaker_description:],
                                                           bert_out.last_hidden_state[n_speaker_description:],
                                                           cur_sentence_indexes_masked)
        ...
        u_vector_fused_by_context = self.context_modeling(
            sentence_vectors_with_convers_shape + self.pos_encoding(sentence_vectors_with_convers_shape),
            src_key_padding_mask=padding_utterance_masked)
        ...
        y_hat = self.output_layer_context(self.dropout_layer(u_vector_fused_by_context)) + self.output_layer(
            self.dropout_layer(sentence_vectors))

        #  intra speaker modeling
        if self.model_configs.intra_speaker_context:

            # padding for masked utterance
            new_padding_utterance_masked = padding_utterance_masked.unsqueeze(-1).repeat(1, 1,
                                                                                         intra_speaker_masekd_all.shape[
                                                                                             -1])
            ...
            u_vector_fused_by_intra_speaker, _ = self.intra_speaker_modeling(i_vector, i_vector, i_vector,
                                                                             attn_mask=(
                                                                                 inter_speaker_masekd_all).repeat(8, 1,
                                                                                                                  1),
                                                                             key_padding_mask=padding_utterance_masked)
            u_vector_fused_by_intra_speaker = u_vector_fused_by_intra_speaker.reshape(
                n_conversation * len_longest_conversation, -1)
            ...
            u_vector_fused_by_inter_speaker, _ = self.inter_speaker_modeling(a_vector, a_vector, a_vector,
                                                                             attn_mask=new_intra_speaker_masekd_all.repeat(
                                                                                 8, 1, 1),
                                                                             key_padding_mask=padding_utterance_masked)
            u_vector_fused_by_inter_speaker = u_vector_fused_by_inter_speaker.reshape(
                n_conversation * len_longest_conversation, -1)

            y_hat = y_hat + self.output_layer_intra(
                self.dropout_layer(u_vector_fused_by_intra_speaker)) + self.output_layer_inter(
                self.dropout_layer(u_vector_fused_by_inter_speaker))

        if self.model_configs.llm_context:
            # llm self_attention modeling
            if self.model_configs.llm_aggregate_method == 'accwr_selfattn':
                llm_vectors_with_attention, attentions = self.llm_attention_modeling(
                    self.llm_query(llm_context_vectors),
                ...
                llm_vectors_with_attention = torch.sum(llm_vectors_with_attention, dim=1) * (
                            1 / torch.sum(~padding_word_masked, dim=1)).unsqueeze(-1)
                llm_context_vectors = llm_vectors_with_attention

            y_hat = y_hat + self.output_llm_context(self.dropout_layer(llm_context_vectors))

        if self.model_configs.speaker_description:
            sp_description_vector_extracted = torch.stack(
                [sp_description_vector[idx] for idx in all_speaker_descriptions], dim=0)
            ...
            if self.model_configs.spdesc_aggregate_method == 'static':
                ...
                utterance_fused_by_characteristic = self.linear_spdesc(
                    sentence_vectors + sp_description_vector_extracted)
                y_hat_characteristic, _ = self.speaker_character_attention_modeling(utterance_fused_by_characteristic,
                                                                                    sp_description_vector,
                                                                                    sp_description_vector)
    ...
    def test_step(self, batch, batch_idx):
        ...
        # 将预测结果转换为情感标签
        pred_emotions = [id2label[pred.item()] for pred in predictions]
    ...
    def on_test_epoch_end(self):
        ...
        # 保存预测结果
        with open('test_predictions_personachat_valid.txt', 'w', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_folder", help="path to data folder", type=str,
                        default='/home/s2220429/per_erc/data/all_raw_data')
    parser.add_argument("--n_best_checkpoint", help="number of best checkpoints to save", type=int, default=1)
    parser.add_argument("--accumulate_grad_batches", help="number of accumulate batch for each grandient update step",
                        type=int, default=2)
    parser.add_argument("--precision",
                        help="precision of computation tensor operators which selected in {32-true, 16-mixed} ",
                        type=str, default="32-true")
    ...
    parser.add_argument("--sent_aggregate_method", help="sent_aggregate_method in  {average, cls, lstm, mlp}", type=str,
                        default="average")
    parser.add_argument("--data_name_pattern", help="data_name_pattern", type=str, default="iemocap.{}window2.json")
    parser.add_argument("--log_dir",
                        help="path of data log dir and trained model. This path is augmented by {dataset_name}",
    ...
    parser.add_argument("--intra_speaker_context", help="use information of intra speaker context", action="store_true",
                        default=False)

    parser.add_argument("--llm_context", help="use llm context or not", action="store_true", default=False)
    parser.add_argument("--llm_aggregate_method",
                        help="method to incoporate llm context vector in {cls, accwr_selfattn, accwr_average}",
                        type=str, default="accwr_average")
    parser.add_argument("--llm_context_file_pattern", help="llm context vector path", type=str,
                        default='iemocap.{}_v2_5')

    parser.add_argument("--speaker_description", help="use llm context or not", action="store_true", default=False)
    parser.add_argument("--spdesc_aggregate_method", help="method to incoporate speaker vector in {static, attn}",
                        type=str, default="static")
    parser.add_argument("--spdesc_enc", help="method to encode speaker description  {roberta, lstm}", type=str,
                        default="roberta")
    parser.add_argument("--speaker_description_file_pattern", help="speaker description path file pattern", type=str,
                        default="iemocap.{}_speaker_descriptions.json")
    ...
    valid_loader = DataLoader(
        BatchPreprocessor.load_raw_data(f"{options.data_folder}/{data_name_pattern.format('valid')}"),
    ...
    train_loader = DataLoader(
        BatchPreprocessor.load_raw_data(f"{options.data_folder}/{data_name_pattern.format('train')}"),
        batch_size=model_configs.batch_size, collate_fn=data_loader_train, shuffle=True)

    data_loader_test = BatchPreprocessor(bert_tokenizer, model_configs=model_configs, data_type='test')
    test_loader = DataLoader(
        BatchPreprocessor.load_raw_data(f"{options.data_folder}/{data_name_pattern.format('test')}"),
    ...
    checkpoint_callback = ModelCheckpoint(dirpath=f"{model_configs.log_dir}",
                                          save_top_k=model_configs.n_best_checkpoint,
    ...
    trainer = Trainer(max_epochs=model_configs.max_ep,
                      ...
                      val_check_interval=0.5 if 'dailydialog' not in options.data_name_pattern else 0.1
                      # 50%/10% epoch - freq time to run evaluate
                      )
