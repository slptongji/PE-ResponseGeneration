...
def get_token_id(tokenizer):
    ...
    query_id, res_id, latent_id, persona_id, emo_id = tokenizer.convert_tokens_to_ids(
        ['<query>', '<response>', '<latent>', '<persona>', '<emotion>'])

    return bos_id, eos_id, pad_id, sep_id, query_id, res_id, latent_id, persona_id, emo_id
...
def create_label(data_file):
    with open(data_file, "r", encoding="utf8") as f:
        emo=[]
        for line in f:
            emo.append(line.strip())
    return emo

def create_encoder_input(per, history,emo_words_list, query_id, res_id, latent_id, persona_id,emo_id, sep_id, eos_id):
    encoder_input_ids = []

    per_input_ids = [latent_id] + [persona_id]
    emo_input_ids = [latent_id] + [emo_id]


    for x in per:
        per_input_ids += x + [sep_id]

    for e in emo_words_list:
        emo_input_ids += e + [sep_id]
    encoder_input_ids += per_input_ids
    encoder_input_ids += emo_input_ids
    # emo_input_ids += per_input_ids
    ...
    emo_attention_mask = [1] * len(emo_input_ids)

    return encoder_input_ids, attention_mask, per_input_ids, per_attention_mask, emo_input_ids, emo_attention_mask
...
def build_dataloader(persona, query, response, cand, emo, tokenizer, max_history=4, n_cand=5, use_all=False):
    bos_id, eos_id, pad_id, sep_id, query_id, res_id, latent_id, persona_id, emo_id = get_token_id(tokenizer)
    dataset = defaultdict(list)
    # i是用户的下标，即有多少个用户
    for i in range(len(persona)):
        persona_ = persona[i]
        per_list = []
        # 循环persona_中的所有persona
        for per in persona_:
            persona_ids = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(per, add_prefix_space=True))
            per_list.append(persona_ids)
            # per_list的长度就是这个用户有多少个persona描述
        query_ = query[i]
        ...
        # 利用emo[i]从情感词典中获取所有的情感词
        emo_index = emo_dictionary[eemo]
        emo_words_list = []
        emo_words = open(f"data/Dictionary/{eemo}.txt","r",encoding='utf-8').readlines()[0].strip().split(',')
        for emo_w in emo_words:
            emo_id_ = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(emo_w,add_prefix_space=True))
            emo_words_list.append(emo_id_)
        # 一个用户有多少个query，用于构造history出来
        for j in range(len(query_)):
            ...
            per_input_ids, per_attention_mask,\
                emo_input_ids, emo_attention_mask = create_encoder_input(per_list, tmp_history,emo_words_list, query_id, res_id,
                                                                     latent_id, persona_id, emo_id, sep_id, eos_id)
            ...
            dataset["emo_input_ids"].append(emo_input_ids)
            dataset["emo_attention_mask"].append(emo_attention_mask)
            ...
            dataset["emolabel"].append([emo_index])
    ...
    for item_name, item in dataset.items():
        if item_name == "input_ids" or item_name == "per_input_ids" or item_name == "emo_input_ids":
            ...
            dataset[item_name] = item
        elif item_name == "attention_mask" or item_name == "decoder_attention_mask" or item_name == "per_attention_mask" or item_name == "emo_attention_mask":
            item = pad_sequence([torch.from_numpy(np.array(x)) for x in item],
            ...
            item = pad_sequence([torch.from_numpy(np.array(x)) for x in item],
                                batch_first=True, padding_value=pad_id)
            dataset[item_name] = item
        elif item_name == "clslabel" or item_name == "emolabel":
