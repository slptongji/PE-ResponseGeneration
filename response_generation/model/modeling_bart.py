...
class LMEDRModel(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, num_token=None, num_latent=10, num_latent2=10, num_latent3=10,emo_num_labels = 7):
        super().__init__(config)
        self.model = BartModel(config)
    ...
    def history_split(self,history, history_emo,history_emo_input_ids,eos_id, sep_id):

        for his in history:
            indices = torch.nonzero(his == eos_id).squeeze()
            sub_tensors = []
            prev_index = 0
            for index in indices:
                sub_tensors.append(his[prev_index:index])
                prev_index = index + 1
            sub_tensors = torch.tensor(sub_tensors)


    def emo_vec(self, his_hidden,his_emo_labels,his_emo_i_hidden, emolabels,res_id, sep_id,persona_id):
        '''
            his_hidden：输入的对话历史编码，query对应的隐藏状态，形状是(batch_size, hidden_size)
            his_emo_labels：对话历史对应的emotion label，emo_q，形状是(batch_size, 1)
            his_emo_i_hidden：对话历史对应的emotion words，emo_words_q（分隔符是pad_id），形状是(batch_size, hidden_size)
            emolabels：是回复对应的情感标签，形状是(batch_size)
        '''
        # 加入seq_length这个维度
        ...
        input_lengths = [len(s) for s in his_hidden]
        lstm_output, lstm_h_n = self.gru(history, sent_len=input_lengths)
        lstm_mask = torch.zeros(lstm_output.size()[0], lstm_output.size()[1]).to("cuda:0")
        ...
        # 计算出回复中表达的情感类别，因为形状是(batch_size, 1, src_len)
        emo_pred = self.tagging(atten_output)

        # 每句话的情感标签
        utt_emo_labels = pad_sequence(emo_his, padding_value=-1, batch_first=True).to("cuda:0")
        dia_mask = torch.ones(utt_emo_labels.size()).long().to("cuda:0")
        ...
        emo_emb = self.emotion_embedding(emo_pred).to("cuda:0")

        # 计算交叉注意力
        ctx_emo = self.emotion_embedding(utt_emo_logits).to("cuda:0")
        atten_outputs, attetion_weights = self.emo_know(ctx_emo, emo_per_input_ids, emo_per_input_ids, None)
    ...
    def forward(
        ...
        emolabel = None,
        emo_per_input_ids=None,
        emo_per_attention_mask=None,
        return_dict=True,
        generate=False,
        ...
        if input_ids is not None:
            ...
            if emo:
                ...
                latent_logits_m3 = self.latent_head_m3(latent_hidden_state_m3)
                emo_latent_variable = torch.mm(torch.softmax(latent_logits_m3,dim=-1), self.memory3)

                # 改进的方法：
                emo_his = None # 这里是要加一个参数传进来的，即获取的对话history中每个utt对应的情感标签
                utt_emo_loss, trg_loss, emo_latent_variable = self.(input_ids, emolabel, emo_his, emo_input_ids)
                # 然后把sos_emb加到latent上面去，以及将两个loss放到返回的模型当中去

            if generate:
                if infer and dialog and emo:
                    return {"latent_variable": dialog_latent_variable + entail_latent_variable + emo_latent_variable, "encoder_outputs": encoder_outputs}
                elif not dialog:
                    return {"latent_variable": entail_latent_variable + emo_latent_variable,  "encoder_outputs": encoder_outputs}
                # 这里是改进方案
                elif not infer:
                    return {"latent_variable": dialog_latent_variable + emo_latent_variable, "encoder_outputs": encoder_outputs}
                elif not emo:
                    return {"latent_variable": entail_latent_variable + dialog_latent_variable, "encoder_outputs": encoder_outputs}
        ...
        if cls_index != None:
            ...
            if len(torch.unique_consecutive(cls_mask.sum(1))) > 1:
                raise ValueError("All examples must have the same number of <eos> tokens.")
            print("cls_index check here")
            print(cls_index)
            sentence_representation = hidden_states[cls_mask, :].view(hidden_states.size(0), -1,hidden_states.size(-1))[:, -1, :]
            print("logits cehck ")
            ...
            emo_logits = emo_logits.view(bs, -1, self.emo_num_labels)
            print(cls_logits.size())
            print(emo_logits.size())
        ...
        if clslabel is not None:
            ...
            # print(bs)
            # print(cls_logits.view(bs, -1).size())
            # print(clslabel.view(-1).size())
        ...
        if input_ids == None:
            ...
            print(cls_logits.size())
            return Seq2SeqLMOutput(
                loss=(masked_lm_loss, cls_loss, emo_loss, m_loss, m_loss_2, infer_masked_lm_loss, bow_loss),
                logits=(lm_logits, cls_logits, emo_logits),
                # 以下是改进方案
                # loss=(masked_lm_loss, cls_loss, emo_loss, m_loss, m_loss_2, infer_masked_lm_loss, bow_loss,utt_emo_loss,trg_loss),
                # logits=(lm_logits, cls_logits, emo_logits),
            )
