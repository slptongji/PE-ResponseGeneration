...
from build_data_PersonaChat import create_data, build_dataloader, build_infer_dataset,create_label
...
def init_config():
    ...
    parser.add_argument("--num_latent2", type=int, default=15, help="number of latent2")
    parser.add_argument("--num_latent3", type=int, default=10, help="number of latent3")
    ...
    parser.add_argument('--cand', type=int, default=8, help='number of candidate')
...
if __name__ == '__main__':
    ...
    add_special_tokens = {'additional_special_tokens': ['<query>', '<response>', '<latent>', '<persona>','<emotion>']}
    args = init_config()
    if args.revised:
        data_from = "_revised"
    else:
    ...
    if args.load_from != None:
        ...
        model = LMEDRModel.from_pretrained("facebook/bart-large", num_labels=1,
                                           ...
                                           num_latent=args.num_latent, num_latent2=args.num_latent2, emo_num_labels=7)
    ...
    emo = create_label(f"data/ConvAI2/train_emo{data_from}.txt")
    train_data = build_dataloader(persona, query, response, cand, emo, tokenizer, max_history=args.max_history, n_cand=args.cand)
    logger.info("Build valid data")
    persona, query, response, cand = create_data(f"data/ConvAI2/valid_self{data_from}.txt")
    emo = create_label(f"data/ConvAI2/valid_emo{data_from}.txt")
    val_data = build_dataloader(persona, query, response, cand,emo, tokenizer, max_history=args.max_history, use_all=True)
    logger.info("Build infer data")
    infer_data = build_infer_dataset(tokenizer, "data/dnli/dialogue_nli_train2.jsonl")

    MODEL_INPUTS = ["input_ids", "attention_mask", "lmlabels", "decoder_input_ids", "decoder_attention_mask",
                    "cls_index", "clslabel", "per_input_ids", "per_attention_mask","emo_input_ids", "emo_attention_mask","emolabel"]
    ...
    for input_name in MODEL_INPUTS:
        if input_name == "clslabel" or input_name == "emolabel":
    ...
    def update(engine, batch):
        ...
        per_input_ids, per_attention_mask, emo_input_ids, emo_attention_mask, emolabel = batch
        ...
        outputs = model(input_ids=input_ids, attention_mask=attention_mask, lmlabels=lmlabels,
                        ...
                        emo_input_ids = emo_input_ids,
                        emo_attention_mask = emo_attention_mask,
                        emolabel = emolabel,
                        )
        (lm_loss, cls_loss,emo_loss, m_loss,m_loss_2, _, bow) = outputs.loss
        loss = lm_loss + cls_loss  + emo_loss + m_loss + m_loss_2+ bow
        ...
        return {'loss': loss.item(), 'cls': cls_loss.item(),'emo':emo_loss.item(), 'lm': lm_loss.item(),
                "mem": m_loss.item(),"mem2": m_loss_2.item(), "bow": bow.item()}
    ...
    def inference(engine, batch):
        ...
        with torch.no_grad():
            ...
            (lm_logits, cls_logits,emo_logits) = outputs.logits
            ...
            # 在train阶段，cls_logits是(bs,cand*num_labels)，emo_logits是(bs,emo_num*num_labels)
            # 在valid阶段，cls_logits是(bs*cand, num_labels)，emo是(bs*cand, num_labels)
            # 而clslabel是(bs)的形状
            cls_logits = cls_logits.view(-1, 20)
            # valid出来之后，要经过处理，处理之后cls_logits要变成(bs,cand*num_labels)
            # 所以emo_logits出来之后也要经过处理，把cand的去掉
            emo_logits = emo_logits.view(-1, 20)
            return (tmp_lmlogits, cls_logits, emo_logits), (lmlabels, clslabel, emolabel)


    evaluator = Engine(inference)

    # 为infer trainer设置事件触发，每个infer的EPOCH结束之后，都会调用trainer的update来训练
    infer_trainer.add_event_handler(Events.EPOCH_COMPLETED, lambda _: trainer.run(train_loader))

    # 每个trainer的EPOCH结束之后，调用evaluator来训练
    ...
    RunningAverage(output_transform=lambda x: x["emo"]).attach(trainer, "emo")
    ...
    RunningAverage(output_transform=lambda x: x["mem2"]).attach(trainer, "mem2")
    ...
    if args.eval:
        ...
        if args.local_rank in [-1, 0]:
            ...
            pbar.attach(trainer, metric_names=["loss", "cls", "emo","lm", "mem","mem2", "bow"])
            ...
            tb_logger.attach(trainer, log_handler=OutputHandler(tag="training",
                                                                    metric_names=["loss", "cls", "emo","lm", "mem","mem2", "bow"]),
                                 event_name=Events.ITERATION_COMPLETED)
